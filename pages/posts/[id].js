        ...
        <div className={`row`}>
          <div className={`${singlePageStyles.mainContent} col-lg-9 col-12`}>
            ...
            <div className={`${singlePageStyles.categoryCard}`}>
              <div className={`${singlePageStyles.smallText}`}> {post.data.attributes.Date} | {post.data.attributes.category.data.attributes.Name}, {post.data.attributes.tags.data.slice(0).map((tag) => {return(<span>{tag.attributes.name}</span>)})} </div>
          ...
          <div className={`${singlePageStyles.sideContent} col-lg-3 col-12`}>
            ...
            {everyPost.data.slice(startSlice, endSlice).map(function (post) {
              ...
              return (
                <div className={`${singlePageStyles.sidecardElement} col-12 px-0`}>
                  ...
                  <div className={`${singlePageStyles.smallText}`}>{post.attributes.Date} | {post.attributes.category.data.attributes.Name}, {post.attributes.tags.data.slice(0).map((tag) => {return(<span>{tag.attributes.name}</span>)})} </div>
